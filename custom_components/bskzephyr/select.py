...
from bskzephyr import FanMode, FanSpeed

SELECT_TYPES: tuple[SelectEntityDescription, ...] = (
    SelectEntityDescription(
        name="Fan Mode",
        key="fanMode",
        options=[m.value for m in FanMode]
    ),
    SelectEntityDescription(
        name="Fan Speed",
        key="fanSpeed",
        options=[s.name for s in FanSpeed]
    )
)
...
class BSKZephyrSelect(SelectEntity, CoordinatorEntity[DeviceDataUpdateCoordinator]):
    def __init__(self, groupID: str, coordinator: DeviceDataUpdateCoordinator, description: SelectEntityDescription):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        self._attr_state = getattr(self.coordinator.data[self.groupID].device, self.entity_description.key)
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        await self.coordinator.api.control_device(self.groupID, **{self.entity_description.key: self._attr_state})
