...
async def async_setup_entry(hass: HomeAssistant, entry: BSKZephyrConfigEntry) -> bool:
    ...
    client = BSKZephyrClient(async_get_clientsession(hass), entry.data[CONF_USERNAME], entry.data[CONF_PASSWORD])
    ...
    coordinator = await hass.async_create_task(async_setup_device_coordinator(hass, entry, client))
